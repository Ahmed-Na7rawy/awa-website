...
import { getFmcgBrandData, FmcgBrandData } from '../data/fmcgBrandsData';
...
export const AwaBrandShowcase: React.FC<AwaBrandShowcaseProps> = ({
  ...
  const currentBrand: FmcgBrandData = getFmcgBrandData(activeBrandId, language);
  ...
  return (
    <div className="fmcg-root">
      ...
      {activeBrandId === 'yalla-drinks' ? (
        ...
        <StickyScrollyHero
          key={activeBrandId + '-hero-' + language}
          brand={currentBrand}
          onOpenQuote={onOpenQuote}
        />
      )}
      ...
      <SkuGrid3D
        key={activeBrandId + '-skus-' + language}
        brand={currentBrand}
        onOpenQuote={onOpenQuote}
      />
      ...
      <BrandStorySplit
        key={activeBrandId + '-story-' + language}
        brand={currentBrand}
        onOpenQuote={onOpenQuote}
      />
      ...
      <BrandFaqAndProof
        key={activeBrandId + '-proof-' + language}
        brand={currentBrand}
        onOpenQuote={onOpenQuote}
      />
        ...
        <div style={{ maxWidth: '1240px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '2rem' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', color: currentBrand.theme.primary, marginBottom: '0.5rem' }}>
              <ShieldCheck size={16} />
              <span>{language === 'ar' ? 'منظومة AWA Group الصناعية والاستهلاكية' : 'AWA Group Industrial & Retail Ecosystem'}</span>
            </div>
            <h4 style={{ fontSize: '1.75rem', fontWeight: 900, color: '#0F172A', marginBottom: '0.35rem' }}>
              {language === 'ar' ? 'عقد شراكة وتوزيع مع AWA Food Solutions' : 'Partner With AWA Food Solutions'}
            </h4>
            <p style={{ fontSize: '0.9rem', color: '#475569', maxWidth: '640px' }}>
              {language === 'ar'
                ? 'نتعاون مع كبرى شركات تصنيع الأغذية، وسلاسل الهايبرماركت والسوبرماركت، وقطاع الفنادق والمطاعم ووكلاء التصدير إقليمياً ودولياً.'
          ...
          >
            <ShoppingBag size={16} />
            <span>{language === 'ar' ? 'تواصل مع فريق المبيعات والتجارة' : 'Connect With Our Trade Team'}</span>
            <ArrowRight size={16} />
          </button>
        </div>
      </section>
