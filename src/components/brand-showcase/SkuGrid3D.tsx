    ...
    <section className="fmcg-sku-section">
      <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1.5rem', marginBottom: '2.5rem' }}>
        <div>
          ...
          >
            <Layers size={14} />
            <span>{language === 'ar' ? 'تشكيلة منتجات التجزئة والتوريد التجاري' : 'Retail & Foodservice SKU Lineup'}</span>
          </div>
          <h2 style={{ fontSize: 'clamp(2rem, 4vw, 2.75rem)', fontWeight: 900, textTransform: 'uppercase', color: 'inherit', letterSpacing: '-0.02em', marginBottom: '0.35rem' }}>
            {language === 'ar' ? `كتالوج منتجات ${brand.brandName}` : `${brand.brandName} PRODUCT CATALOG`}
          </h2>
          <p style={{ fontSize: '0.95rem', opacity: 0.75, margin: 0 }}>
            {language === 'ar'
              ? 'استكشف أحجام العبوات، ومؤشرات قوة النكهة، والمواصفات الفنية للتجزئة.'
      ...
      <div style={{
        ...
        <div>
          <h4 style={{ fontSize: '1.3rem', fontWeight: 900, color: 'inherit', marginBottom: '0.25rem' }}>
            {language === 'ar' ? 'هل تبحث عن توكيل توزيع تجاري أو توريد للفنادق والكافيهات؟' : 'Looking for Commercial Distribution or Foodservice Supply?'}
          </h4>
          <p style={{ fontSize: '0.875rem', opacity: 0.75 }}>
            {language === 'ar'
              ? 'نقوم بتوريد سلاسل الهايبرماركت، ومحلات التجزئة، وسلاسل الكافيهات المتخصصة، وشركاء التصدير حول العالم.'
        ...
        >
          <ShoppingBag size={16} />
          <span>{language === 'ar' ? 'طلب عرض سعر تجاري B2B' : 'Request B2B Trade Pricing'}</span>
          <ArrowRight size={16} />
        </button>
      </div>
    </section>
