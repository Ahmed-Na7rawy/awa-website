...
export const BrandFaqAndProof: React.FC<BrandFaqAndProofProps> = ({ brand }) => {
    ...
    <section className="fmcg-proof-section">
      ...
      <div style={{ marginBottom: '5rem' }}>
        <div style={{ textAlign: 'center', maxWidth: '760px', margin: '0 auto 3rem auto' }}>
          ...
          >
            ...
            <span>{language === 'ar' ? 'آراء وتقييمات موثقة من خبراء الصناعة' : 'Verified Industry Reviews'}</span>
          </div>
          <h3 style={{ fontSize: 'clamp(2rem, 3.5vw, 2.75rem)', fontWeight: 900, textTransform: 'uppercase', color: '#0F172A', letterSpacing: '-0.02em', marginBottom: '0.5rem' }}>
            {language === 'ar' ? 'ثقة الطهاة والباريستا والمستهلكين' : 'TRUSTED BY CHEFS, BARISTAS & CONSUMERS'}
          </h3>
          <p style={{ fontSize: '1rem', color: '#475569', lineHeight: 1.6 }}>
            {language === 'ar'
              ? `اكتشف لماذا تعد منتجات ${brand.brandName} الخيار الأول في كبرى سلاسل الهايبرماركت ومحلات التجزئة والمطاعم والكافيهات.`
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', color: brand.theme.primary, marginBottom: '0.5rem' }}>
          <Store size={16} />
          <span>{language === 'ar' ? 'التوافر في منافذ التجزئة والهايبرماركت' : 'Nationwide Retail Availability'}</span>
        </div>
        <h4 style={{ fontSize: '1.75rem', fontWeight: 900, color: '#0F172A', marginBottom: '0.5rem' }}>
          {language === 'ar' ? `أين تجد منتجات ${brand.brandName}` : `WHERE TO FIND ${brand.brandName.toUpperCase()}`}
        </h4>
        <p style={{ fontSize: '0.95rem', color: '#475569', maxWidth: '640px', margin: '0 auto 2.5rem auto' }}>
          {language === 'ar'
            ? 'متوفر في كبرى سلاسل الهايبرماركت والسوبرماركت والمتاجر المتخصصة في مصر وأسواق التصدير الإقليمية.'
      ...
      <div className="fmcg-faq-container">
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <h3 style={{ fontSize: 'clamp(1.85rem, 3vw, 2.35rem)', fontWeight: 900, color: '#0F172A', textTransform: 'uppercase' }}>
            {language === 'ar' ? 'هل لديك استفسارات؟ لدينا كافة الإجابات.' : 'GOT QUESTIONS? WE’VE GOT ANSWERS.'}
          </h3>
        </div>
